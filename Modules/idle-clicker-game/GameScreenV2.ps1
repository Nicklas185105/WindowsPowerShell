...
class GameScreen {
    ...
    # Keep references to UI labels that need dynamic updates:
    ...
    [void] Show([object]$gameData, [object]$buildings) {
        ...
        ##########################################################
        $leftPanel = New-Object Terminal.Gui.FrameView "Stats"
        $leftPanel.X = 0
        $leftPanel.Y = 0
        $leftPanel.Width = $this.LeftWidth
        $leftPanel.Height = [Terminal.Gui.Dim]::Fill()

        # Title and Name labels.
        $titleLabel = New-Object Terminal.Gui.Label ($this.CenterText("Factory", $this.LeftWidth))
        $titleLabel.X = 1; $titleLabel.Y = 1
        $leftPanel.Add($titleLabel)

        $nameLabel = New-Object Terminal.Gui.Label ($this.CenterText("Name", $this.LeftWidth))
        $nameLabel.X = 1; $nameLabel.Y = 2
        $leftPanel.Add($nameLabel)

        # Separator line.
        $sepLabel = New-Object Terminal.Gui.Label ($this.LeftBorder)
        $sepLabel.X = 0; $sepLabel.Y = 3
        $leftPanel.Add($sepLabel)

        # Points
        $pointsStr = $this.FormatLargeNumber($gameData.Clicks) + " p"
        $this.PointsLabel = New-Object Terminal.Gui.Label ($this.CenterText($pointsStr, $this.LeftWidth))
        $this.PointsLabel.X = 1; $this.PointsLabel.Y = 4
        $leftPanel.Add($this.PointsLabel)

        # Production rate
        $idleStr = $this.FormatLargeNumber($gameData.IdleIncome) + " p/s"
        $this.IdleLabel = New-Object Terminal.Gui.Label ($this.CenterText($idleStr, $this.LeftWidth))
        $this.IdleLabel.X = 1; $this.IdleLabel.Y = 6
        $leftPanel.Add($this.IdleLabel)

        # Click button
        # $clickButton = New-Object Terminal.Gui.Button "Click Me"
        # # Position it at the bottom center of your main window.
        # $clickButton.X = 1; $clickButton.Y = 10
        # # Wire up the Clicked event. When clicked, it increments the global Clicks value.
        # $clickButton.Accepting += {
        #     param($sender, $e)
        #     try {
        #         # Increase points by the click value.
        #         $localData.Clicks += $localData.ClickValue
        #     }
        #     catch {
        #         Write-Host "Error in Accepting event: $_"
        #     }
        #     # Ensure that the event is not canceled.
        #     $e.Cancel = $false
        # }
        ...
        $labelKeys = New-Object Terminal.Gui.Label ("Keys".PadRight($this.LeftWidth))
        $labelKeys.X = 0; $labelKeys.Y = $keysY
        $leftPanel.Add($labelKeys)
        $keysY++

        $labelSpace = New-Object Terminal.Gui.Label ("$($global:Keys.Space.Name): Click".PadRight($this.LeftWidth))
        $labelSpace.X = 0; $labelSpace.Y = $keysY
        $leftPanel.Add($labelSpace)
        $keysY++

        $labelReset = New-Object Terminal.Gui.Label ("$($global:Keys.R.Name): Reset game".PadRight($this.LeftWidth))
        $labelReset.X = 0; $labelReset.Y = $keysY
        $leftPanel.Add($labelReset)
        $keysY++

        $labelSave = New-Object Terminal.Gui.Label ("$($global:Keys.S.Name): Save game".PadRight($this.LeftWidth))
        $labelSave.X = 0; $labelSave.Y = $keysY
        $leftPanel.Add($labelSave)
        $keysY++

        $labelLoad = New-Object Terminal.Gui.Label ("$($global:Keys.L.Name): Load game".PadRight($this.LeftWidth))
        $labelLoad.X = 0; $labelLoad.Y = $keysY
        $leftPanel.Add($labelLoad)
        $keysY++

        $labelQuit = New-Object Terminal.Gui.Label ("$($global:Keys.Escape.Name): Quit game".PadRight($this.LeftWidth))
        $labelQuit.X = 0; $labelQuit.Y = $keysY
        $leftPanel.Add($labelQuit)
        $keysY++

        $labelCtrlC = New-Object Terminal.Gui.Label ("(don't do ctrl+C)".PadRight($this.LeftWidth))
        $labelCtrlC.X = 0; $labelCtrlC.Y = $keysY
        $leftPanel.Add($labelCtrlC)
        $keysY += 5

        # Last save info.
        $labelLastSave = New-Object Terminal.Gui.Label ("Since last save".PadRight($this.LeftWidth))
        ...
        $this.LastSaveLabel = New-Object Terminal.Gui.Label ($timeStr.PadRight($this.LeftWidth))
        ...
        $shopPanelLeft = New-Object Terminal.Gui.FrameView "Shop Column 1"
        $shopPanelLeft.X = [Terminal.Gui.Pos]::At($this.LeftWidth + 1)
        $shopPanelLeft.Y = 0
        $shopPanelLeft.Width = $this.RightWidth
        $shopPanelLeft.Height = [Terminal.Gui.Dim]::Fill()

        # Second right column panel
        $shopPanelRight = New-Object Terminal.Gui.FrameView "Shop Column 2"
        $shopPanelRight.X = [Terminal.Gui.Pos]::At($this.LeftWidth + $this.RightWidth + 2)
        $shopPanelRight.Y = 0
        $shopPanelRight.Width = $this.RightWidth
        $shopPanelRight.Height = [Terminal.Gui.Dim]::Fill()
        ...
        for ($i = 0; $i -lt $midpoint; $i++) {
            ...
            $label1 = New-Object Terminal.Gui.Label ($shopInfo[0])
            $label1.X = 1; $label1.Y = $yPos
            $shopPanelLeft.Add($label1)
            $yPos++
            $label2 = New-Object Terminal.Gui.Label ($shopInfo[1])
